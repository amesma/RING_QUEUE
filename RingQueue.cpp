...
public:
	...
	public:
		...
		bool operator==(const iterator& rhs) const {
			...
			// Replace the line(s) below with your code.
			return ((rhs.offset == offset) && (rhs.parent == parent) && (rhs.parent->begin_index == parent->begin_index));
		}
...
public:
	// Constructor
	RingQueue() : begin_index(0), ring_size(0), end_in(0) { }
	...
	ItemType back() const {
		...
		// Replace the line(s) below with your code.
		return buffer[end_in + 1];
	}

	// Mutators
	void push_back(const ItemType& value) {
		size_t my_end = (begin_index + ring_size) % MAX_SIZE;
